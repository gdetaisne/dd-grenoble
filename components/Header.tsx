...
import { getCityData, getCityDataFromUrl } from '@/lib/cityData';
...
function resolveCitySlug(): string {
  if (typeof window !== 'undefined') {
    try {
      ...
      return getCityDataFromUrl(url).slug;
    } catch {
  ...
  try {
    return getCityDataFromUrl(env.SITE_URL).slug;
  } catch {
    return 'toulouse';
  }
}
...
export default function Header() {
  ...
  const cityLabel = useMemo(() => {
    if (typeof window !== 'undefined') {
      ...
      if (hostname.includes('toulousain')) return 'Toulousain';
      if (hostname.includes('parisien')) return 'Parisien';
    }
  ...
  const zonesItems = useMemo(
    () => [
      { href: `/${city.slug}/`, label: `Déménagement ${city.nameCapitalized}` },
      { href: `/quartiers-${city.slug}/`, label: 'Tous les quartiers' },
    ],
    [city]
  );
          ...
          >
            ...
            {isZonesDropdownOpen && (
              <div className="absolute top-full left-0 mt-2 w-64 bg-white/95 backdrop-blur rounded-lg shadow-lg border border-white/20 py-2">
                ...
                {city.corridors.slice(0, 3).map((corridor) => (
                  <Link
                    key={corridor.slug}
                    href={`/${city.slug}-vers-${corridor.slug}`}
                    className="block px-4 py-2 text-gray-800 hover:bg-white/50 transition-colors"
                    onClick={() => {
                      setIsZonesDropdownOpen(false);
                      trackClick(`corridor-${corridor.slug}`);
                    }}
                  >
                    {city.nameCapitalized} → {corridor.destination}
                  </Link>
                ))}
              </div>
            )}
