...
function InventaireIAPageInner() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Ici, Grenoble encode encore l'adresse complète dans originAddress/destinationAddress.
      // On reconstruit une adresse simple "CP Ville" si on reçoit CP + Ville.
      if (originPostalParam || originCityParam) {
        const originParts = [];
        if (originPostalParam) originParts.push(originPostalParam);
        if (originCityParam) originParts.push(originCityParam);
        const originAddress = originParts.join(' ');
        if (originAddress) {
          nextState = { ...nextState, originAddress };
        }
      }

      if (destPostalParam || destCityParam) {
        const destParts = [];
        if (destPostalParam) destParts.push(destPostalParam);
        if (destCityParam) destParts.push(destCityParam);
        const destinationAddress = destParts.join(' ');
        if (destinationAddress) {
          nextState = { ...nextState, destinationAddress };
        }
      }
